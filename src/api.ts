...
api.post("/api/sync/cloudflare", requireAdmin, async (c) => {
  const registry = await loadRegistry(config.registryDir, config.baseDomain);
...
api.delete("/api/domains/:name/cloudflare", requireAdmin, async (c) => {
  const name = c.req.param("name");
...
// for debugging
api.post("/api/sync/cloudflare/dry-run", requireAdmin, async (c) => {
  const registry = await loadRegistry(config.registryDir, config.baseDomain);
